...
public class makeQuizController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (answer == null) {
            ...
            Question q = new Question(question,date.toString() ,idUser );
            dao.addQuiz(q);
            int idQues = dao.getIdQuiz();

            for (int i = 1; i <=option.length; i++) {
                boolean checkAs = false;
                for (String a : answer) {
                ...
                if(checkAs){
                    dao.addAnswer(new Answer(idQues,option[i-1],1,i));
                }else{
                     dao.addAnswer(new Answer(idQues,option[i-1],0,i));
                }
            }
